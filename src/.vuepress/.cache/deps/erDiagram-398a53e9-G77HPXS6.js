...
// node_modules/.pnpm/mermaid@10.3.1/node_modules/mermaid/dist/erDiagram-398a53e9.js
var ct = function() {
  var t = function(M, a, l, d) {
    for (l = l || {}, d = M.length; d--; l[M[d]] = a)
      ;
    return l;
  }, r = [1, 2], e = [1, 5], n = [6, 9, 11, 23, 25, 27, 29, 30, 31, 52], s = [1, 17], p = [1, 18], f = [1, 19], o = [1, 20], c = [1, 21], _ = [1, 22], E = [1, 25], g = [1, 30], O = [1, 31], y = [1, 32], A2 = [1, 33], Y = [1, 34], U = [6, 9, 11, 15, 20, 23, 25, 27, 29, 30, 31, 44, 45, 46, 47, 48, 52], X = [1, 46], I = [30, 31, 49, 50], N = [4, 6, 9, 11, 23, 25, 27, 29, 30, 31, 52], v = [44, 45, 46, 47, 48], k2 = [22, 37], x = [1, 66], P = [1, 65], Z = [22, 37, 39, 41], w = {
    trace: function() {
    },
    yy: {},
...
var pt = function(t) {
  return j[t] === void 0 && (j[t] = { attributes: [] }, k.info("Added new entity :", t)), j[t];
};
var Ft2 = () => j;
var Wt = function(t, r) {
  let e = pt(t), n;
  for (n = r.length - 1; n >= 0; n--)
    e.attributes.push(r[n]), k.debug("Added attribute ", r[n].attributeName);
};
var Vt = function(t, r, e, n) {
  ...
  lt.push(s), k.debug("Added new relationship :", s);
};
var Ut = () => lt;
var zt = function() {
  j = {}, lt = [], La();
...
var oe = (t, r, e) => {
  ...
  e.forEach((N) => {
    ...
    if (E = Math.max(E, z.width), g = Math.max(g, S2.width), k2 = Math.max(z.height, S2.height), c) {
      ...
      O = Math.max(O, l.width), k2 = Math.max(k2, l.height);
    }
    if (_) {
      ...
      y = Math.max(y, a.width), k2 = Math.max(k2, a.height);
    }
    w.height = k2, o.push(w), A2 += k2 + n * 2, Y += 1;
  });
  let U = 4;
  c && (U += 2), _ && (U += 2);
  const X = E + g + O + y, I = {
  ...
  if (e.length > 0) {
    ...
    let v = f.height + n * 2, k2 = "attributeBoxOdd";
    o.forEach((x) => {
      const P = v + n + x.height / 2;
      x.tn.attr("transform", "translate(" + s + "," + P + ")");
      const Z = t.insert("rect", "#" + x.tn.node().id).classed(`er ${k2}`, true).attr("x", 0).attr("y", v).attr("width", E + s * 2 + N).attr("height", x.height + n * 2), w = parseFloat(Z.attr("x")) + parseFloat(Z.attr("width"));
      x.nn.attr(
        "transform",
        "translate(" + (w + s) + "," + P + ")"
      );
      const z = t.insert("rect", "#" + x.nn.node().id).classed(`er ${k2}`, true).attr("x", w).attr("y", v).attr("width", g + s * 2 + N).attr("height", x.height + n * 2);
      ...
      if (c) {
        ...
        const M = t.insert("rect", "#" + x.kn.node().id).classed(`er ${k2}`, true).attr("x", S2).attr("y", v).attr("width", O + s * 2 + N).attr("height", x.height + n * 2);
        S2 = parseFloat(M.attr("x")) + parseFloat(M.attr("width"));
      }
      _ && (x.cn.attr(
        "transform",
        "translate(" + (S2 + s) + "," + P + ")"
      ), t.insert("rect", "#" + x.cn.node().id).classed(`er ${k2}`, "true").attr("x", S2).attr("y", v).attr("width", y + s * 2 + N).attr("height", x.height + n * 2)), v += x.height + n * 2, k2 = k2 === "attributeBoxOdd" ? "attributeBoxEven" : "attributeBoxOdd";
    });
  } else
...
var de2 = function(t, r, e, n, s) {
  ...
  const p = e.edge(
    ...
    return A2.y;
  }).curve(Cf), o = t.insert("path", "#" + n).classed("er relationshipLine", true).attr("d", f(p.points)).style("stroke", R.stroke).style("fill", "none");
...
var ue = function(t, r, e, n) {
  R = Ft().er, k.info("Drawing ER diagram");
  ...
  Fa(o, A2, y, R.useMaxWidth), o.attr("viewBox", `${O.x - g} ${O.y - g} ${y} ${A2}`);
};
...
//# sourceMappingURL=erDiagram-398a53e9-G77HPXS6.js.map
