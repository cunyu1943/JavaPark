...
// node_modules/.pnpm/mermaid@10.3.1/node_modules/mermaid/dist/timeline-definition-a91b186e.js
var Y = function() {
  var n = function(k2, r, a, h) {
    for (a = a || {}, h = k2.length; h--; a[k2[h]] = r)
      ;
    return a;
...
var ot = function() {
  D.length = 0, Z.length = 0, W = "", B.length = 0, La();
};
var ht = function(n) {
  W = n, D.push(n);
};
...
var mt = function() {
  ...
  function t(i, o, l, p, g, d, f, S, _) {
    ...
    for (let k2 = 0; k2 < I.length; k2++) {
      const r = k2 * b - b * (I.length - 1) / 2, a = o.append("text").attr("x", l + g / 2).attr("y", p).attr("fill", _).style("text-anchor", "middle").style("font-size", b).style("font-family", v);
      a.append("tspan").attr("x", l + g / 2).attr("dy", r).text(I[k2]), a.attr("y", p + d / 2).attr("dominant-baseline", "central").attr("alignment-baseline", "central"), s(a, f);
    }
  }
...
var jt = function(n, t, e, s) {
  ...
  k.debug("task", f), R.initGraphics(d);
  const _ = s.db.getSections();
  k.debug("sections", _);
  let b = 0, v = 0, I = 0, k2 = 0, r = 50 + o, a = 50;
  k2 = 50;
  let h = 0, u = true;
  _.forEach(function(C) {
    ...
    k.debug("sectionHeight before draw", $), b = Math.max(b, $ + 20);
  });
  let y = 0, c = 0;
  k.debug("tasks.length", f.length);
  for (const [C, w] of f.entries()) {
    ...
    k.debug("taskHeight before draw", m), v = Math.max(v, m + 20), y = Math.max(y, w.events.length);
    let N = 0;
    for (let A2 = 0; A2 < w.events.length; A2++) {
      const K = {
        descr: w.events[A2],
        section: w.section,
        number: w.section,
  ...
  k.debug("maxSectionHeight before draw", b), k.debug("maxTaskHeight before draw", v), _ && _.length > 0 ? _.forEach((C) => {
    const w = f.filter((A2) => A2.section === C), $ = {
      number: h,
      descr: C,
      section: h,
    ...
    k.debug("sectionNode output", N), m.attr("transform", `translate(${r}, ${k2})`), a += b + 50, w.length > 0 && rt(
      d,
      w,
      h,
      r,
      ...
      b,
      false
    ), r += 200 * Math.max(w.length, 1), a = k2, h++;
  }) : (u = false, rt(
    d,
    f,
    h,
  ...
  k.debug("bounds", L), S && d.append("text").text(S).attr("x", L.width / 2 - o).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), I = u ? b + v + 150 : v + 100, d.append("g").attr("class", "lineWrapper").append("line").attr("x1", o).attr("y1", I).attr("x2", L.width + 3 * o).attr("y2", I).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), Gg(
    void 0,
    d,
...
var rt = function(n, t, e, s, i, o, l, p, g, d, f) {
  ...
  for (const _ of t) {
    ...
    k.debug("taskNode", b);
    const v = n.append("g").attr("class", "taskWrapper"), k2 = R.drawNode(v, b, e, l).height;
    if (k.debug("taskHeight after draw", k2), v.attr("transform", `translate(${s}, ${i})`), o = Math.max(o, k2), _.events) {
...
//# sourceMappingURL=timeline-definition-a91b186e-ERPAU7VT.js.map
