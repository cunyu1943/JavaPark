...
// node_modules/.pnpm/mermaid@10.3.1/node_modules/mermaid/dist/svgDraw-3570afd5.js
var S2 = 0;
var O = function(i, e, t, n, r) {
  ...
  const l = e.points, g = S().x(function(d) {
    ...
    return d.y;
  }).curve(Cf), s = i.append("path").attr("d", g(l)).attr("id", "edge" + S2).attr("class", "relation");
  let p = "";
  ...
  if (c % 2 !== 0 && c > 1) {
    ...
    k.debug("cardinality_1_point " + JSON.stringify(d)), k.debug("cardinality_2_point " + JSON.stringify(w)), u = d.x, m = d.y, B = w.x, T = w.y;
  }
  if (t.title !== void 0) {
  ...
  k.info("Rendering relation " + JSON.stringify(t)), t.relationTitle1 !== void 0 && t.relationTitle1 !== "none" && i.append("g").attr("class", "cardinality").append("text").attr("class", "type1").attr("x", u).attr("y", m).attr("fill", "black").attr("font-size", "6").text(t.relationTitle1), t.relationTitle2 !== void 0 && t.relationTitle2 !== "none" && i.append("g").attr("class", "cardinality").append("text").attr("class", "type2").attr("x", B).attr("y", T).attr("fill", "black").attr("font-size", "6").text(t.relationTitle2), S2++;
};
var P = function(i, e, t, n) {
  k.debug("Rendering class ", e, t);
  const r = e.id, o = {
    id: r,
    label: e.id,
    width: 0,
...
`);
  s.forEach(function(c) {
    k.debug(`Adding line: ${c}`), g.append("tspan").text(c).attr("class", "title").attr("dy", t.textHeight);
  });
...
var M = function(i) {
  ...
  if (h > 1 && a > h && a <= i.length) {
    ...
    if (e = r + b + "(" + cd(u.trim()) + ")", a < i.length) {
      let m = i.substring(a + 1, a + 2);
      t === "" && !m.match(g) ? (t = C(m), n = i.substring(a + 2).trim()) : n = i.substring(a + 1).trim(), n !== "" && (n.charAt(0) === ":" && (n = n.substring(1).trim()), n = " : " + cd(n), e += n);
    }
  } else
    e = r + cd(i);
  return {
...
//# sourceMappingURL=chunk-UFBL5X6U.js.map
